...
We are aware modern C++ is way different in a lot of aspects. So if you want to become a proficient C++ developer, it's up to you to go further after this modules.
...
- However, **you can't use any other external library.** It means C++11 (and derived forms) and Boost libraries are **forbidden**. The following functions are **forbidden** too: `*printf()`, `*alloc()` and `free()`.
...
- You are allowed to use the STL only in **Modules 08 and 09**. That means: no Containers (`vector`/`list`/`map`, and so forth) and no Algorithms (anything that requires including the `<algorithm>` header) until then.
