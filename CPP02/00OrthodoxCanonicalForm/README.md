...
#### Objective
Create your first fixed-point number class in Orthodox Canonical Form.

#### Requirements
- Implement a `Fixed` class in Orthodox Canonical Form.
- Add private members:
  - An integer storing the fixed-point raw value.
  - A static constant integer storing the number of fractional bits, always set to `8`.
- Add public members:
...
- Respect the expected constructor/destructor/log messages from the subject.
...
- **Public members:**
  ...
  - A destructor.
  - A member function `int getRawBits( void ) const;` that returns the raw value of the fixed-point number.
